...
async def market_home(uid, qz, param=""):
    ...
    lines = [
        "##### 🏮 修真坊市",
        "> 全服道友托管交易。出售物品与收购灵石均由系统锁定，成交自动结算。",
        f"> 当前在售 **{sell_count}** 单｜收购 **{buy_count}** 单｜挂单时效 **{MARKET_EXPIRE_HOURS} 小时**",
        "***",
        _buttons(("坊市列表", "浏览最新挂单"), ("坊市 搜 ", "搜索物品*"), ("坊市 分类 ", "按分类浏览*")),
        "",
        _buttons(("坊市上架 ", "上架：物品 数量 单价*"), ("坊市收购 ", "收购：物品 单价 数量*")),
        "",
        _buttons(("我的摊位", "我的摊位"), ("坊市交易记录", "交易记录"), ("坊市帮助", "坊市规则")),
    ]
...
async def market_help(uid, qz):
    lines = [
        "##### 🏮 坊市帮助",
        "**成交与保管**",
        f"> 出售单上架即托管物品；收购单会预存全部灵石。成交时从卖家收入扣除 **8%** 手续费（向下取整并销毁）。",
        f"> 所有订单有效期为 **{MARKET_EXPIRE_HOURS} 小时**；出售余货或收购余款会自动返还，撤摊也会原路退回。",
        "**交易限制**",
        "> 仅可交易背包中的可堆叠通用物品；绑定、任务、轮回专属道具不可上架。出售单不能自行购买，收购单不能自行交付。",
        "**指令**",
        "> 坊市 搜 物品名｜坊市 分类 丹药｜坊市上架 物品名 数量 单价",
        "> 坊市购买 摊位号 数量｜坊市收购 物品名 单价 数量｜坊市出售 收购单号 数量",
        "> 我的摊位｜撤摊 摊位号｜坊市底价 物品名｜坊市交易记录",
        "***",
        _buttons(("坊市", "坊市首页"), ("坊市列表", "浏览挂单")),
    ]
    return {"type": "markdown", "content": "\n".join(lines)}
...
async def show_market_menu(uid, qz):
    ...
    lines = [
        "##### 🏮 坊市菜单",
        "> 玩家之间的全服托管交易。出售余货与收购余款会在 72 小时到期后自动返还；卖家成交收入扣除 8% 手续费。",
        "",
        _buttons(("坊市", "坊市首页"), ("坊市列表", "浏览最新挂单"), ("坊市 搜 ", "搜索物品*")),
        "",
        _buttons(("坊市 分类 ", "分类：丹药/材料*"), ("坊市上架 ", "上架：物品 数量 单价*")),
        "",
        _buttons(("坊市购买 ", "购买：摊位号 数量*"), ("坊市收购 ", "收购：物品 单价 数量*")),
        "",
        _buttons(("坊市出售 ", "交付收购单*"), ("我的摊位", "我的摊位"), ("坊市交易记录", "交易记录")),
        "",
        _buttons(("坊市帮助", "坊市规则"), ("主菜单", "主菜单")),
    ]
    return {"type": "markdown", "content": "\n".join(lines)}
