...
# ================================
# 设计原则：
# 1. 依据 2026-08-04 最近 1813 场单人战斗校准，避免跨档属性突增
# 2. 档次内通过线性插值平滑过渡
# 3. 玩家Lv.N的裸装属性应略高于Lv.N副本小怪，穿装备后能稳定推进
#
...
#
# 60级旧配置相较50级出现攻击+164%、气血+533%的异常断层；现改为渐进曲线。
# Boss与高阶普通怪的最终倍率由 data_monster 控制，并由 p3 平衡迁移限制上限。

DUNGEON_BASE_STATS = {
    # min_level: (攻击, 防御, 气血, 速度, 暴击, 暴伤, 闪避, 命中, 破防, 吸血)
    1: (550, 380, 2800, 65, 700, 3000, 600, 3000, 100, 0),
    10: (650, 450, 3500, 70, 900, 3200, 750, 3500, 200, 0),
    20: (750, 520, 4200, 76, 1100, 3400, 900, 4200, 320, 100),
    30: (880, 612, 6000, 85, 1300, 3600, 1000, 4400, 410, 200),
    40: (1350, 920, 9000, 98, 1500, 3800, 1100, 4800, 550, 300),
    50: (1800, 1150, 12500, 108, 1700, 4000, 1200, 5300, 650, 400),
    60: (1850, 1200, 13500, 115, 1900, 4200, 1250, 5600, 700, 450),
    70: (2050, 1350, 15500, 125, 2100, 4400, 1300, 6000, 800, 500),
    80: (2250, 1500, 18000, 135, 2300, 4600, 1400, 6400, 900, 550),
    90: (2500, 1650, 21000, 145, 2500, 4800, 1500, 6800, 1000, 600),
}
