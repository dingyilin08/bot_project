...
async def show_update_log(uid):
    """显示当前线上版本的玩家可见更新内容。"""
    output = "##### 📜 更新日志｜v1.22\n\n"
    output += "**⚔️ 协作战斗与丹道完善**\n> 队伍战斗采用统一回合快照；90 秒未行动的队友自动防御。炼丹支持保守/均衡/冒险火候、品质产量与耐药衰减。\n\n"
    ...
    output += "**🌌 世界Boss与赛季开放**\n> 世界 Boss 提供挑战、辅助、净化三类贡献及幂等奖励；副本、宗门委托、世界 Boss 会自动累积赛季经验，奖励仅为外观/称号资格。\n\n"
    output += "**👥 菜单体验整理**\n> 队伍、灵兽、洞府、宗门与赛季操作已分类收拢；主菜单只保留系统入口。\n\n"
    output += "**🛍️ 体验改进**\n> 群聊快捷入口统一改为参数输入按钮，按钮展示名称与实际执行指令分离，减少误操作。\n\n"
    output += "**📦 部署提示**\n> 本次功能需要先执行新增的 P1/P2 数据库迁移脚本后再启用。\n\n"
