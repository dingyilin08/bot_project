...
```html
<!-- 回车指令：点击后直接发送（无需参数的完整指令使用） -->
<qqbot-cmd-enter text="实际发送的指令" />
<!-- 参数指令：点击后插入输入框，玩家补充参数后发送（带后缀参数的指令使用） -->
<qqbot-cmd-input text="实际发送的指令" show="按钮显示文字" />
```

- 无后缀指令、空参有默认值的指令（如各类背包/列表）、参数已填完整的指令 → 用 `qqbot-cmd-enter`
- 必须玩家补充后缀参数的指令（text 尾空格预留参数位、show 带 `*` 或参数示例）→ 用 `qqbot-cmd-input`
- 注意：`qqbot-cmd-enter` 仅支持 `text` 属性，且群聊和文字子频道不支持（仅单聊可用）
